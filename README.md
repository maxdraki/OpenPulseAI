...
- [x] Tauri v2 desktop wrapper — Rust spawns the UI/API server as a persistent sidecar (`src-tauri/src/server_sidecar.rs`) and keeps it alive; the webview talks to it directly over `fetch`. System tray + close-to-hide. Not yet E2E tested with a real display server — see [`docs/desktop-testing-checklist.md`](docs/desktop-testing-checklist.md) for the manual verification steps.
