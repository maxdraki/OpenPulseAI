...
1. Run `gh auth status >/dev/null 2>&1 || echo "!! gh not authenticated — run: gh auth login"; printf '%s\n' "{{github_repo_urls}}" | grep -vE '^\{\{|^[[:space:]]*$' | tr ',' '\n' | sed 's/[[:space:]]//g; s/\.git$//' | grep -v '^$' | while IFS= read -r url; do host=$(echo "$url" | sed 's|https://||; s|/.*||'); repo=$(echo "$url" | sed 's|https://[^/]*/||'); [ -z "$repo" ] && continue; HARG=""; [ "$host" != "github.com" ] && HARG="--hostname $host"; commits=$(gh api $HARG --paginate "repos/$repo/commits?per_page=100&since={{since_iso}}" --jq '.[] | {sha: .sha[0:7], msg: .commit.message, author: .commit.author.name, date: .commit.author.date}' 2>/dev/null); prs=$(gh api $HARG --paginate "repos/$repo/pulls?state=all&per_page=100&sort=updated&direction=desc" --jq '.[] | select(.updated_at > "{{since_iso}}") | {number, title, state, user: .user.login, updated: .updated_at, body: (.body // "" | .[0:1500])}' 2>/dev/null); issues=$(gh api $HARG --paginate "repos/$repo/issues?state=all&per_page=50&sort=updated&direction=desc&since={{since_iso}}" --jq '.[] | select(.pull_request == null) | {number, title, state, user: .user.login, updated: .updated_at, body: (.body // "" | .[0:1500])}' 2>/dev/null); comments=$(gh api $HARG --paginate "repos/$repo/issues/comments?sort=updated&direction=desc&since={{since_iso}}&per_page=50" --jq '.[] | {issue: (.issue_url | split("/") | last), user: .user.login, updated: .updated_at, body: (.body | .[0:800])}' 2>/dev/null); releases=$(gh api $HARG --paginate "repos/$repo/releases?per_page=30" --jq '.[] | select(.published_at != null and .published_at > "{{since_iso}}") | {name, tag: .tag_name, published: .published_at, body: (.body // "" | .[0:1000])}' 2>/dev/null); [ -z "$commits$prs$issues$comments$releases" ] && continue; echo "=== $repo ==="; [ -n "$commits" ] && { echo "--- Commits ---"; echo "$commits"; }; [ -n "$prs" ] && { echo "--- PRs updated ---"; echo "$prs"; }; [ -n "$issues" ] && { echo "--- Issues updated ---"; echo "$issues"; }; [ -n "$comments" ] && { echo "--- Issue comments ---"; echo "$comments"; }; [ -n "$releases" ] && { echo "--- Releases ---"; echo "$releases"; }; done; reviewed=$(gh api --paginate "search/issues?q=reviewed-by:@me+updated:>{{since_date}}&per_page=50" --jq '.items[]? | {number, title, repo: (.repository_url | split("/") | .[-2:] | join("/")), url: .html_url, updated: .updated_at, state, body: (.body // "" | .[0:600])}' 2>/dev/null); commented=$(gh api --paginate "search/issues?q=commenter:@me+updated:>{{since_date}}&per_page=50" --jq '.items[]? | {number, title, repo: (.repository_url | split("/") | .[-2:] | join("/")), url: .html_url, updated: .updated_at, state, body: (.body // "" | .[0:600])}' 2>/dev/null); if [ -n "$reviewed$commented" ]; then echo "=== my-cross-repo-activity ==="; [ -n "$reviewed" ] && { echo "--- PRs I reviewed ---"; echo "$reviewed"; }; [ -n "$commented" ] && { echo "--- Issues/PRs I commented on ---"; echo "$commented"; }; fi; true` to collect commits, PRs, issues, issue comments, reviews, and releases since the last run (with full commit messages, PR bodies, issue bodies, and comment text). Repos and the cross-repo section are OMITTED entirely when they produced no output (no activity, or an error such as a 404 for a private repo the active account can't see) — so an inactive or inaccessible repo never becomes an empty theme.
