...
When commands 6 and 7 return per-repo data, organise that output first, grouped by repo (one level-3 heading per repo), with commits and open PRs for each. If both commands produce no output, fall back to summarising the general activity from commands 1–5 as before.
