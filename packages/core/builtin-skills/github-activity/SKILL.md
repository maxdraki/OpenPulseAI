...
1. Run `printf '%s\n' "{{github_repo_urls}}" | grep -vE '^\{\{|^[[:space:]]*$' | tr ',' '\n' | sed 's/[[:space:]]//g; s/\.git$//' | grep -v '^$' | while IFS= read -r url; do host=$(echo "$url" | sed 's|https://||; s|/.*||'); repo=$(echo "$url" | sed 's|https://[^/]*/||'); [ -z "$repo" ] && continue; echo "=== $repo ==="; if [ "$host" = "github.com" ]; then gh api "repos/$repo/commits?per_page=50&since={{since_iso}}" --jq '[.[] | {sha: .sha[0:7], message: (.commit.message | split("\n")[0]), author: .commit.author.name, date: .commit.author.date}]' 2>/dev/null; gh api "repos/$repo/pulls?state=all&per_page=20&sort=updated" --jq '[.[] | select(.updated_at > "{{since_iso}}") | {number, title, state, user: .user.login, updated: .updated_at}]' 2>/dev/null; else gh api "repos/$repo/commits?per_page=50&since={{since_iso}}" --hostname "$host" --jq '[.[] | {sha: .sha[0:7], message: (.commit.message | split("\n")[0]), author: .commit.author.name, date: .commit.author.date}]' 2>/dev/null; gh api "repos/$repo/pulls?state=all&per_page=20&sort=updated" --hostname "$host" --jq '[.[] | select(.updated_at > "{{since_iso}}") | {number, title, state, user: .user.login, updated: .updated_at}]' 2>/dev/null; fi; done; true` for commits since last run and recent PRs in your watched repos
...
- **Pull Requests**: PRs updated since last run. If none, write "No PR activity since last run."
