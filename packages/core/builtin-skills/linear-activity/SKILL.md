...
Linear uses a GraphQL API. All queries go to the same endpoint with POST.

1. Run `curl -s -H "Authorization: {{linear_api_key}}" -H "Content-Type: application/json" -X POST https://api.linear.app/graphql -d '{"query":"{ viewer { name } organization { name } issues(first: 50, orderBy: updatedAt, filter: { updatedAt: { gte: \"'$(date -u -v-1d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -d '1 day ago' +%Y-%m-%dT%H:%M:%SZ)'\" } }) { nodes { identifier title state { name type } priority assignee { name } labels { nodes { name } } updatedAt createdAt } } }"}'` to get recently updated issues
